...
const LongBar = artifacts.require('LongBar');

contract('LongToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.long = await LongToken.new({ from: alice });
        this.bar = await LongBar.new(this.long.address, { from: alice });
        this.long.mint(alice, '100', { from: alice });
        this.long.mint(bob, '100', { from: alice });
        this.long.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.long.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.long.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.long.approve(this.bar.address, '100', { from: alice });
        await this.long.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.long.balanceOf(this.bar.address)).valueOf(), '30');
        // LongBar get 20 more LONGs from an external source.
        await this.long.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more LONGs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.long.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.long.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.long.balanceOf(bob)).valueOf(), '98');
    });
});
