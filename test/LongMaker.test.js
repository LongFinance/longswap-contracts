...
const LongMaker = artifacts.require('LongMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('LongMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.long = await LongToken.new({ from: alice });
        await this.long.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await LongMaker.new(this.factory.address, bar, this.long.address, this.weth.address);
        this.longWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.long.address)).logs[0].args.pair);
    ...
    it('should make LONGs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.longWETH.address, '10000000', { from: minter });
        await this.long.transfer(this.longWETH.address, '10000000', { from: minter });
        await this.longWETH.mint(minter);
        ...
        // After calling convert, bar should have LONG value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.long.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for LONG-ETH pair
        await this.long.transfer(this.longWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.longWETH.address, '100000', { from: minter });
        await this.longWETH.sync();
        await this.long.transfer(this.longWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.longWETH.address, '10000000', { from: minter });
        await this.longWETH.mint(minter);
        assert.equal((await this.longWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.long.address, this.weth.address);
        assert.equal((await this.long.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.longWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});